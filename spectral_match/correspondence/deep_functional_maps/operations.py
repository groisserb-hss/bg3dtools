"""
TensorFlow operations for deep functional maps.

Provides differentiable least-squares solvers, correspondence matrix
construction, and residual network building blocks for learned
...
def correspondence_matrix(sigs, evecs_t):
    A, B = (tf.matmul(e, s) for (e, s) in zip(evecs_t, sigs))
    C = solve_lstsq(A, B)
    return C


def soft_correspondence_ensemble(C, evecs_1_t, evecs_2):
...
def geodesic_error_ensemble(P, dist_1, dist_2):
