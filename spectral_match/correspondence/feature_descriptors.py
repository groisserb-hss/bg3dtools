"""
Spectral feature descriptors for mesh surfaces.

Computes Wave Kernel Signatures (WKS), Heat Kernel Signatures (HKS),
and Gaussian curvature descriptors from Laplacian eigendecompositions.
"""
...
def wave_kernel_signature(evecs, evals, eps, sigma):
    ...
    # temp = cdist(np.log(evals).reshape(-1, 1), eps.reshape(-1, 1))
    # assert np.allclose(dE, temp)
    dE = cdist(np.log(evals).reshape(-1, 1), eps.reshape(-1, 1))
...
class DescriptorClass:
    def __init__(self, sig_config):
        emin, emax, num_wks, num_hks, num_gaussian, num_signatures = sig_config
        assert num_signatures >= num_wks + num_hks + num_gaussian  # leave room for other signatures in subclasses
